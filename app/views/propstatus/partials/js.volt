...
function updated(id) {
    var form = $('form[name="propstatus_edit"]').attr('action', '{{ url('propstatus/updated/') }}' + id);
    $.ajax({
        type: 'GET',
        url: '{{ url('propstatus/detail/') }}'+id,
        dataType:'json',
        success: function(response) {
            console.log(response);
            var $form = $('form#propstatus_edit');      
...
function clear_form(id){
    $('form[name="propstatus"]').find('[name]').val('');
}
</script>