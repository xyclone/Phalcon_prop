...
function updated(name) {
    var form = $('form[name="projtype_edit"]').attr('action', '{{ url("projtypes/updated/") }}' + name);
    $.ajax({
        type: 'GET',
        url: '{{ url("projtypes/detail/") }}'+name,
        dataType:'json',
        success: function(response) {
            console.log(response);
            var $form = $('form#projtype_edit');      
...
function clear_form(name){
    $('form[name="projtypes"]').find('[name]').val('');
}
</script>