...
function deleted(name, fieldname) {
    $('input#name_delete').val(name);
    $('span#field').text(fieldname);
}

function updated(name) {
    var form = $('form[name="proptype_edit"]').attr('action', '{{ url("proptypes/updated/") }}' + name);
    $.ajax({
        type: 'GET',
        url: '{{ url("proptypes/detail/") }}'+name,
        dataType:'json',
        success: function(response) {
...
function clear_form(id){
    $('form[name="proptypes"]').find('[name]').val('');
}
</script>