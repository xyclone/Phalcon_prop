...
function updated(name) {
    var form = $('form[name="tenure_edit"]').attr('action', '{{ url("tenures/updated/") }}' + name);
    $.ajax({
        type: 'GET',
        url: '{{ url("tenures/detail/") }}'+name,
        dataType:'json',
        success: function(response) {
            console.log(response);
            var $form = $('form#tenure_edit');      
...
function clear_form(name){
    $('form[name="tenures"]').find('[name]').val('');
}
</script>