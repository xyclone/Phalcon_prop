...
function updated(name) {
    var form = $('form[name="agency_edit"]').attr('action', '{{ url("agencies/updated/") }}' + name);
    $.ajax({
        type: 'GET',
        url: '{{ url("agencies/detail/") }}'+name,
        dataType:'json',
        success: function(response){
            var $form = $('form#agency_edit');      
...
function clear_form(id){
    $('form[name="agencies"]').find('[name]').val('');
}
</script>