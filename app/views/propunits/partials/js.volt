...
function updated(id) {
    var form = $('form[name="propunit_edit"]').attr('action', '{{ url("propunits/updated/") }}' + id);
    $.ajax({
        type: 'GET',
        url: '{{ url("propunits/detail/") }}'+id,
        dataType:'json',
        success: function(response) {
            console.log(response);
            var $form = $('form#propunit_edit');      
...
function clear_form(name){
    $('form[name="propunits"]').find('[name]').val('');
}
</script>